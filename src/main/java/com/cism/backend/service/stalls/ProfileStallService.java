...
public class ProfileStallService {
        ...
        private OwnerStallDto.TrendDto calculateRevenueTrend(StallModel stall) {
                java.math.BigDecimal currentPeriod = createStallIncomesRepository
                                .sumIncomeByStallAndDateRange(stall,
                ...
                java.math.BigDecimal previousPeriod = createStallIncomesRepository
                                .sumIncomeByStallAndDateRange(stall,
                                                java.time.Instant.now().minus(java.time.Duration.ofDays(14)),
                                                java.time.Instant.now().minus(java.time.Duration.ofDays(7)))
                                .orElse(java.math.BigDecimal.ZERO);
                ...
                if (previousPeriod.compareTo(java.math.BigDecimal.ZERO) > 0) {
                        ...
                        percentageChange = diff.divide(previousPeriod, 4, java.math.RoundingMode.HALF_UP).doubleValue()
                                        * 100;
        ...
        private OwnerStallDto.MealsModel mapMeal(StallItemModel m) {
                return new OwnerStallDto.MealsModel(
                                ...
                                m.getCreatedAt(),
                                m.getUpdatedAt(),
                                m.getItemVariations() != null
                                                ? m.getItemVariations().stream().map(this::mapVariation1).toList()
                                                : java.util.List.of());
        }
        ...
        private OwnerStallDto.SnacksModel mapSnacks(StallItemModel s) {
                return new OwnerStallDto.SnacksModel(
                                ...
                                s.getCreatedAt(),
                                s.getUpdatedAt(),
                                s.getItemVariations() != null
                                                ? s.getItemVariations().stream().map(this::mapVariation1).toList()
                                                : java.util.List.of());
        }
        ...
        private OwnerStallDto.DrinksModel mapDrinks(StallItemModel d) {
                return new OwnerStallDto.DrinksModel(
                                ...
                                d.getCreatedAt(),
                                d.getUpdatedAt(),
                                d.getItemVariations() != null
                                                ? d.getItemVariations().stream().map(this::mapVariation1).toList()
                                                : java.util.List.of());
        }

        private ItemVariationsResponse mapVariation1(ItemVariationsModel v) {
